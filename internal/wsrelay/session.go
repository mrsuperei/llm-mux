...
func (s *session) run(ctx context.Context) {
	...
	for {
		...
		// Handle binary messages (optimized path for stream chunks)
		if msgType == websocket.BinaryMessage && IsBinaryMessage(data) {
			s.dispatchBinary(data)
			continue
		}

		// Handle JSON messages (standard path)
		var msg Message
		if err := json.Unmarshal(data, &msg); err != nil {
			s.manager.logDebugf("wsrelay: invalid json message: %v", err)
			continue
		}
		s.dispatch(msg)
	}
...
func (s *session) dispatchBinary(data []byte) {
	binMsg := DecodeBinaryChunk(data)
	if binMsg.ID == "" {
		return
	}

	value, ok := s.pending.Load(binMsg.ID)
	if !ok {
		return
	}
	...
	// Convert binary type to message type
	var msgType string
	switch binMsg.Type {
	case BinaryTypeStreamChunk:
		msgType = MessageTypeStreamChunk
	case BinaryTypeStreamEnd:
		msgType = MessageTypeStreamEnd
	case BinaryTypeStreamStart:
		msgType = MessageTypeStreamStart
	default:
		return
	}
	...
	if len(binMsg.Data) > 0 {
		msg.Payload = map[string]any{"data": string(binMsg.Data)}
	}
	...
	if msgType == MessageTypeStreamEnd {
		if actual, loaded := s.pending.LoadAndDelete(binMsg.ID); loaded {
